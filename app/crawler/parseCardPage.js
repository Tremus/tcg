...
const parseCardPage = htmlBody => {
    const table =
        '<table' +
        htmlBody.split(/<table\sclass="wikitable"/)[1].split(/<\/table/)[0] +
        '</table>';
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (i === 0) {
            const name = /\s([a-zA-Z0-9,_\-%\s]+)/.exec(
            ...
            const srcDisplayUrl =
                e.children[0].children[0].children[0].children[0].children[0]
                    .children[0].src;
            if (!srcDisplayUrl) {
            ...
            !cardDetails.civilization &&
            e.innerHTML.match(/title="Civilization">/)
        ) {
            const civilization = /(Light|Water|Darkness|Fire|Nature)/.exec(
                e.children[1].children[0].innerHTML,
            )[1];
            if (!civilization) {
            ...
            !cardDetails.type &&
            e.innerHTML.match(/title="Card\sType">/)
        ) {
            // (creature / spell)
            const type = e.children[1].children[0].innerHTML;
            if (!type) {
            ...
            e.innerHTML.match(/title="Mana\sCost">/)
        ) {
            ...
            !cardDetails.race &&
            cardDetails.type === 'Creature' &&
            e.innerHTML.match(/title="Race">/)
        ) {
            const race = e.children[1].children[0].innerHTML;
            if (!race) {
            ...
            e.innerHTML.match(/title="Text">English\sText/)
        ) {
            var abilities = [];
            const lines = e.children[1].innerHTML
                .split(/<p>|<\/p>/)
                .filter(t => !!t);
            ...
            e.innerHTML.match(/title="Flavor\sText">/)
        ) {
            ...
            !cardDetails.manaNumber &&
            e.innerHTML.match(/title="Mana\sNumber">/)
        ) {
            ...
            e.innerHTML.match(/title="Artist">/)
        ) {
            ...
            const setsrarity = e.children[0].children[0].children[0].innerHTML.split(
                '<br>',
            );
